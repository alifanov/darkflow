...
Шаг 3 слияния с Deckbook: общий текст вынесен из команд, на каждой команде появился frontmatter.

- **Workflow** — P9: блок «Step 1 — Read project config» (6–8 строк) был скопирован в 20 команд. Контракт конфига описан один раз в `.darkflow.d/claude.md` (что запускать, где кэш, все ключи и значения по умолчанию), в команде осталась одна строка с перечнем ключей, которые она реально использует. Минус 116 строк дублирования.
- **Workflow** — P2: правило терсности («How to write») — тоже один раз в `.darkflow.d/claude.md`, а не копией в каждую команду. Главное в нём: чистый прогон не пишет ничего. Никаких «в целом всё в порядке» и пустых секций ради доказательства, что рутина отработала — молчание и есть результат.
- **Updated routine** — P1: frontmatter на всех 23 командах — `description` в одну строку и `allowed-tools`. Описание теперь видно в списке команд, а не только первой строкой тела. Пуш объявлен только у `fix-issues` и `fix-ci-issue`. Оговорка: `allowed-tools` не умеет «весь Bash кроме `git push`», а аудиты запускают произвольные инструменты проекта, так что запрет пуша для аудитов держится на правиле A7, а не на frontmatter — объявление тут декларативное.
...
- **Docs** — housekeeping получил третью операцию: вернуть рабочую копию на базовую ветку. `fix-issues` обязан переключаться обратно сам, но оборванная сессия оставляет HEAD на фиче-ветке — и все последующие записи по A7 уходили бы не туда; чинить задачу, не починив чекаут, значит исправить половину. Незакоммиченные изменения не трогаются.
- **Docs** — уточнён порог P4: чистый прогон не пишет секцию вовсе, поэтому счёт идёт по прогонам (`RoutineLog`), а не по файлам логов, и прогон без секции обрывает серию. P1 переформулирован — граница проходит по `push`, а не по `write`: писать и коммитить может каждый аудит, пушить только fix-рутины. В шаге 10 установка Dark Flow передвинута перед выгрузкой документов, иначе I1 предложил бы заархивировать только что импортированное.

## [4.22.6] — 2026-08-11

- **Docs** — третий круг критики `docs/deckbook-merge.md`. Добавлено A10: одна сессия на проект. Машинный семафор ограничивает только общее число сессий (3), но пускает две рутины одного проекта в один чекаут — а worktree правилами запрещены; вместе с A7 («рутины коммитят сами») это гонка за `index.lock` и риск утащить чужие правки в коммит. Лечится per-project локом перед общим слотом, тем же атомарным `noclobber`, что уже в воркере; занятый проект пропускает тик.
- **Docs** — контент Deckbook теперь переносится, а не бросается: документы каждого проекта выгружаются из MCP в файлы под раскладку A4, открытые задачи заводятся в Dark Flow (`todo`/`in_progress` → `approved`, `needs_human` → `proposed`, комментарии сворачиваются в тело). Закрытые задачи остаются историей в БД Deckbook. Порядок внутри проекта: сначала контент, пока MCP ещё подключён, потом установка, потом снятие задач и сервера.
- **Docs** — порядок работ вырос до 11 шагов и начинается с M1 (пересобрать список модулей): C2 сливает рутины из разных модулей, D2/D4 два модуля убивают, поэтому до согласования списка ничего удалять нельзя. Раздел «Known gap» упразднён — дыра стала первым шагом. Зафиксированы риск нагрузки на локальный воркер после переезда восьми проектов (регулируется `maxConcurrent`, составом рутин и кронами уже по факту) и то, что теряется вместе с Deckbook: MCP-интерфейс к задачам и 28 vitest-файлов.

## [4.22.5] — 2026-08-11

- **Docs** — `docs/deckbook-merge.md`: добавлено A9 — удаление или переименование команды снимает и её рутину, из каталога `routines.ts` и из `RoutineConfig` всех проектов. Иначе воркер по расписанию зовёт `/darkflow:<удалённая>` вечно; касается и слияний C1/C2/C4, где старые имена рутин уходят вместе со старыми командами. Раздел «Retiring Deckbook» дополнен инвентаризацией: на Deckbook сидят восемь проектов (secscanner, mailmonitor, pageradar, qabot, mystize, sqlformatter, deckbook, scopegate) плюс `deckbook-private` — девять MCP-серверов и 142 scheduled-задачи. Порядок работ вырос до девяти шагов: шаг 8 — перевести проекты на Dark Flow по одному (установка, перенос рутин, снятие задач и MCP), шаг 9 — удалить `/deckbook:*`, когда не останется ни одного.

## [4.22.4] — 2026-08-11

- **Docs** — второй круг критики `docs/deckbook-merge.md`, исправлены фактические ошибки плана. «Предложение → `needsHuman`» было прямым переносом слова из Deckbook: у нас этот флаг значит «агент упёрся» и взаимоисключим с `approved`, поэтому предложение идёт в `proposed`, в очередь триажа. Слитая security-рутина зафиксирована как еженедельная на Opus — осознанная плата: алерт Dependabot ждёт до 7 дней вместо суток. Ротация логов отменена совсем (она же и подрывала порог «3 лога подряд» — история уезжала в архив). Добавлено A7: рутины сами коммитят то, что пишут — сейчас снапшоты копятся некоммиченными и утекают в чужую ветку. Добавлено A8: один архив `docs/_archive/` вместо двух разных путей. Появился раздел «Retiring Deckbook» (удаляем `/deckbook:*` и одноимённые scheduled-задачи, БД не трогаем) и порядок работ из 8 шагов — общий текст выносится до слияния команд, миграция доков идёт последней. Известная дыра оставлена явно: модули установщика после C2/D2/D4 надо пересобрать.

## [4.22.3] — 2026-08-11

- **Docs** — ревизия `docs/deckbook-merge.md` после критики плана: убрано всё, что требовало достраивать инфраструктуру ради самого плана. Отменены перенос комментариев в отдельную таблицу (миграция без пользы) и дельта прогонов из `RoutineLog` (там нет признака успеха — «с прошлого раза» стартовало бы от упавшего прогона). Три housekeeping-рутины схлопнуты в один суточный bash-проход в воркере: ни возврат зависших задач, ни уборка worktree, ни ротация логов не требуют рассуждения — это работа для `ci-watch`-механики за ноль токенов. `allowed-tools` больше не обещает read-only аудиты: по A3 каждый аудит пишет в дневной лог, а ограничить `Write` одной папкой нельзя. Таблицы расписания в `README.md` и `install.sh` удаляются, а не генерируются. Опись задач — флаг `--format compact`, а не новая команда. В БД остаётся одно новое поле (staging URL), ключевые страницы берутся из `sitemap.xml`. Переезд на `logs/` + `state/` разбит на три шага, миграция установленных проектов — последним.

## [4.22.2] — 2026-08-11

- **Docs** — `docs/deckbook-merge.md`: закрыты все 22 открытых пункта по командам и установщику. Ключевое: `routines/` удаляется целиком (расписание руками записано в пяти местах, реальны только каталог и БД); четыре команды выбрасываются (`grill`, `claude-md-update`, `csp-setup`, `product-overview`); `security-audit`+`vulnerability-check` и `architecture-review`+`code-health` сливаются, три дизайн-команды становятся `check-design`+`check-ux`; добавляются `unstick-tasks`, `clean-worktrees`, `rotate-logs`. Паспорт продукта отдельным документом заводить не будем — расширяем `Project`/`config.json`; дельта прогонов берётся из `RoutineLog`, а не из парсинга markdown. Итого команд 27 → ~20.

## [4.22.1] — 2026-08-11

- **Docs** — заведён `docs/deckbook-merge.md`: рабочий журнал решений по слиянию Deckbook в Dark Flow. Зафиксировано принятое (документы остаются текстом в репозитории, комментарии отдельной таблицей, дневной лог, раскладка `state/`, правила поведения в командах, расписание на своём воркере), отклонённое (MCP, дерево подзадач, сроки/рекуррентность, токены-агенты, аутентификация) и 20 открытых пунктов по командам и установщику. Имплементации нет — только решения.
...
- **`CI_WAIT_TIMEOUT` по умолчанию 600 → 60 с.** Поймано на догфудинге: в репозитории без push-триггерного CI (сам darkflow) `ci-wait.sh` честно возвращал `2`, но **через 10 минут** — то есть правило «дождись вердикта» стоило бы десяти минут простоя на каждом пуше в таких проектах. Таймаут относится только к **появлению** рана, а он регистрируется за считаные секунды после пуша; сколько ран потом будет выполняться, этой переменной не ограничено — там ждёт `gh run watch`. Замер: 65 с вместо 600 с, зелёный путь (scopegate) не изменился.

## [4.21.1] — 2026-07-30

- **Правило «After each push» доехало до проектов.** В 4.21.0 оно было добавлено в `templates/darkflow/claude.md` — а это **образец**, а не источник: реальный `.darkflow.d/claude.md` собирает `generate_darkflow_md()` внутри `install.sh`, и переустановка правило бы не принесла. Секция добавлена в генератор.
- Раскатано по 14 подключённым проектам (`install.sh --yes`, без `--force`). `.darkflow.d/claude.md` перегенерируется безусловно, поэтому `--force` не нужен — а он вдобавок затирал бы локально изменённые `docs/*` в каждом репозитории.
- `~/.darkflow/ci-wait.sh` проверен на живом CI: scopegate (`darkflow-ci-gate`) → `GREEN`, exit `0`; darkflow (push-триггерного CI нет) → exit `2`; ран Dependabot на том же SHA до фильтра давал `RED`, после фильтра по `event` отбрасывается.

## [4.21.0] — 2026-07-30

- **`~/.darkflow/ci-wait.sh` — синхронная проверка CI в той же сессии, что и пуш.** До сих пор вердикт по CI приходил только асинхронно: пуш → уведомление от GitHub или задача от `ci-watch` через час. Сессия, вызвавшая падение, к тому моменту закрыта. Хелпер ждёт ран GitHub Actions **того коммита, который только что запушили** (матч по `headSha`, не «последний ран» — иначе ловится предыдущий), и возвращает вердикт кодом выхода: `0` — зелёный или CI нет, `1` — красный (печатает URL джобы и хвост `--log-failed`), `2` — рана за таймаут не появилось. Таймаут — `CI_WAIT_TIMEOUT`, по умолчанию 600 с.
  - Фильтр по `event` (`push`/`pull_request`/`workflow_dispatch`/`merge_group`) — тот же класс ложняков, что закрыт в `ci-watch` v4.20.0: раны Dependabot (`event: dynamic`) висят на том же SHA ветки и без фильтра дают красный вердикт, не имеющий отношения к коммиту. Поймано на первом же прогоне в самом darkflow. Раны с `conclusion` `skipped`/`cancelled` тоже не вердикт.
  - Список ранов перечитывается через 5 с после появления первого: второй workflow на том же пуше регистрируется на такт позже, и без повтора он проскочил бы мимо проверки как ложный зелёный.
- **`fix-issues` больше не закрывает задачу вслепую.** После пуша — вызов `ci-wait.sh`; на `1` задача не закрывается, а уходит в `needs-human` с URL упавшей джобы. `2` трактуется как «в проекте нет CI на этот пуш» и не блокирует.
- **Правило «After each push» в шаблоне `CLAUDE.md`** (и в `.darkflow.d/claude.md` самого darkflow) — любая сессия, которая пушит, обязана дождаться вердикта, а не расходиться.
- `install.sh` кладёт `ci-wait.sh` в `~/.darkflow/` рядом с `get-config.sh` и `df`.

`ci-wait.sh` не заменяет `ci-watch`, а дополняет его: один и тот же сигнал, но первый — пока сессия ещё открыта, второй — как страховка для всего, что запушили мимо агента.

## [4.20.2] — 2026-07-29

- **`ci-watch` — красные раны старше `CI_STALE_DAYS` (7) больше не считаются.** Рутина брала свежайший ран на каждый workflow и репортила его, если `conclusion=failure`, без оглядки на дату. Если workflow упал однажды и с тех пор не запускался, это не сигнал, а история: перезапустить его нечему, значит позеленеть он не может, и заведённая задача **не закрывается никогда**. Поймано на живом кейсе: у naturalwrite на ветке `dev` единственный не-Dependabot workflow — `CodeQL Security Analysis`, чей последний ран упал **2 июня** и больше не запускался; `ci-watch` держал репозиторий красным вечно и завёл незакрываемую задачу #289. Детект зависших ранов фильтр не трогает — ран, который висит в `queued` два месяца, отметить как раз надо.
- Добавлен хелпер `epoch_to_iso()` — обратный к `iso_to_epoch()`. Отдельная функция, а не `epoch_fmt()`, потому что тот форматирует в **локальном** времени: пометить его вывод суффиксом `Z` означало бы молча сдвинуть порог на смещение машины от UTC (здесь — на 2 часа). Таймстемпы GitHub — UTC ISO-8601, поэтому сравнение строк корректно работает как сравнение дат.
- `--self-test` расширен: UTC-корректность и round-trip `epoch_to_iso` ↔ `iso_to_epoch`, плюс фикстура на то, что двухмесячный красный ран отбрасывается, а свежий остаётся.
- **Гейт `darkflow-ci-gate` больше не пишет в GitHub Issues** (правка в 7 репозиториях: qabot, scopegate, mailmonitor, secscanner, pageradar, vargi, adbrief). Шаги `Open issue on failure` / `Close issue on green` вели в канал, который никто не читает: task store с v4.0.0 в своей базе, а джоба внутри GitHub Actions не может позвать локальный `df`. Теперь `conclusion` воркфлоу читает `ci-watch` и сам заводит задачу, забирая детали падения из лога рана (`gh run view --log-failed`). Убраны также право `issues: write` и переменные `ISSUE_STATUS` / `ISSUE_PRIORITY`, шапка переписана под фактическую механику. Минус 88 строк в каждом файле; проверки `lint` + `test` и `runs-on` не тронуты.

## [4.20.1] — 2026-07-29

- **`ci-watch` — локальная проба сведена к `pnpm lint`, тесты убраны.** Первый же реальный прогон на 8 проектах дал `local test` красным у **7 из 8** — и все семь оказались ложняками: `pnpm test` падал из окружения воркера, тогда как интерактивно в том же чекауте тесты зелёные (scopegate 382/382, sqlformatter 18/18). Причина у vitest-проектов — не разрешался опциональный нативный биндинг `@rolldown/binding-darwin-arm64`; суите вообще нужны БД, env-переменные и сгенерированные клиенты, которых фоновому демону предоставлять неоткуда. Каждое такое падение — отчёт об окружении, а не о коде, и каждое заводило фантомную задачу с `priority=high`. Тесты остались там, где окружение объявлено: в CI-workflow проекта (за ним следит проба A) и в `fix-issues`, который прогоняет их на переднем плане перед пушем. `lint` оставлен — он герметичен, статический анализ по рабочему дереву даёт один вердикт где угодно, и он сразу дал истинное срабатывание (openhumancy: 156 ошибок).
- Закрыты 5 фантомных задач, заведённых v4.20.0 (sqlformatter #32, scopegate #209, mailmonitor #470, secscanner #658, qabot #863); в 3 задачи со смешанным содержимым (openhumancy #200, naturalwrite #289, adsynex #254) добавлен комментарий, что пункт `local test` игнорировать.
- **Цикл подтверждён на живом кейсе:** adsynex — `ci-watch` завёл задачу #254 в 10:07 по красному `CI` (конфликт версий pnpm), `fix-issues` починил и запушил, CI зелёный в 10:19, задача закрыта. 12 минут от красного пайплайна до зелёного без человека.

## [4.20.0] — 2026-07-29

- **Новая рутина — `ci-watch`: у `source:ci` задач снова есть producer.** v4.0.0 удалил workflow `darkflow-ci-gate.yml` из шаблонов («ран в GitHub Actions и не достаёт до локального webapp»), а v4.3.0 вернул рутину-*потребителя* `fix-ci-issue`, читающую task store. Producer'а не осталось ни одного: **последняя задача с `source:ci` создана 30 июня**, при этом `fix-ci-issue` за следующие 30 дней отработал 3335 раз и каждый раз отрапортовал «ok» на пустой очереди. Красный CI перестал превращаться в задачу вообще.
  - **`ci-watch` — чистый bash внутри воркера, без агента, без токенов, без слота конкурентности.** Решение «CI красный» и запись задачи — это бухгалтерия, а не рассуждение; думает потом `fix-ci-issue`. Диспетчер обрабатывает рутину in-process (`ci_watch`), поэтому у неё нет и не должно быть command-файла — добавлены исключения в `preflight()`, в `mode_watch` и в `mode_manual`, иначе guard «нет command-файла → skip» вырезал бы её молча.
  - **Проба A — GitHub Actions по базовой ветке.** Берётся *свежайший ран на каждый workflow* (старый красный ран, чей workflow уже позеленел, не должен держать задачу открытой) и флагуется, если он `conclusion=failure` **или всё ещё висит в `queued`/`in_progress` больше 45 минут** (`CI_STUCK_MIN`). Второй случай — не меньшая дыра, чем первый: когда self-hosted раннер уходит в offline, джоба никогда не красная **и** никогда не зелёная, и репозиторий выглядит здоровым просто потому, что молчит. Раны с `event == "dynamic"` (собственные апдейт-джобы Dependabot) отфильтрованы: они падают штатно, а уникальное имя у каждого (`npm_and_yarn in /. for next - Update #149…`) ломает дедуп по имени workflow и залило бы задачу десятком фантомных «упавших workflow», которые не могут позеленеть никогда. Обновления зависимостей — работа `vulnerability-check`.
  - **Проба B — локальные `lint` + `test`** (`pnpm`, плюс `ruff check .` для Python) в корне проекта, но только когда `HEAD` сдвинулся с прошлого зелёного прогона — на простаивающей репе проба бесплатна. Это единственное, что покрывает проекты **вообще без GitHub-workflow**, где смотреть в Actions нечего. Пропускается при отсутствии `node_modules`: установка зависимостей — не дело этой рутины, а отсутствующий `node_modules` выглядел бы как падение кода. Маркер HEAD (`state/ci-watch.sha`) пишется **только когда локальные проверки прошли**, поэтому красная репа продолжает перепроверяться и не «зеленеет» по факту неизменившегося коммита.
  - **Одна дедуплицированная задача на ветку** (`CI failure on <branch>`, `source=ci`, `priority=high`, `status=approved`) с URL ранов и последними 40 строками каждого лога падения. Если задача уже открыта — комментарий вместо дубликата. Когда всё позеленело — задача **закрывается**, и именно это делает надёжным счётчик попыток в `fix-ci-issue` (макс. 3 на одну открытую задачу).
  - **Core-рутина, намеренно НЕ под модулем `ci-gate`.** Модуль гейтил self-hosted раннер, которого больше нет; опциональным тут быть нечему — нет ни агента, ни настройки сверх авторизованного `gh`. К тому же модуль включён лишь у 2 проектов из 10, так что под ним рутина не досталась бы как раз тем репам, где CI и лежит. Задачи создаются `status=approved`, поэтому их подхватывает либо `fix-ci-issue` (когда модуль включён — с ограниченными 3 попытками), либо обычный `fix-issues` наравне с остальной approved-очередью.
  - Добавлен хелпер `iso_to_epoch()` — BSD и GNU `date` расходятся в каждом нужном для этого флаге, а `python3` и так жёсткая зависимость воркера.
  - `--self-test` расширен: парсер таймстемпов + офлайн-фикстура на триаж ранов (позеленевший workflow не репортится, `queued` попадает в stuck-кандидаты, `dynamic` отфильтрован).
- **Docs** — `docs/ci-runner.md` переписан: вместо требований к self-hosted раннеру и Dockerfile для образа — сравнительная таблица «раннер против воркера» и инструкция по миграции (удалить `.github/workflows/darkflow-ci-gate.yml`, снять раннер, включить `ci-watch`). `routines/fix-ci-issue.md` больше не называет producer'ом workflow гейта.
...
- **Workflow** — сужена роль ADR после появления реестра гипотез: правило «Decision made based on data → ADR» в `agent-workflow.md` раздваивало фиксацию data-вердиктов между `decisions/` и Closed-таблицей `product/hypotheses.md`. Теперь ADR пишется только для решений-ограничений (архитектура, стек, стратегия — то, что запрещает/предписывает будущие изменения); обычные data-вердикты (эксперимент подтверждён/опровергнут, кампания выключена) остаются в реестре без ADR. Соответствующая приписка добавлена в шаблон `hypotheses.md`.
