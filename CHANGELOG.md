...
### Installer — воркер видит все тулы, которыми пользуется, и не молчит о протухшем plist
- **Installer** — `_launchd_path()` собирал PATH только из каталогов `claude` и `node`, поэтому
  `gh`, `jq`, `python3`, `pnpm`, `ruff` из `/opt/homebrew/bin` воркеру под launchd были
  недоступны. Ни одна из пропаж не роняла ран: вызывающий уходил в ветку «тула нет» и сообщал
  состояние, которого не наблюдал. Теперь PATH пробирается по каждому бинарю, который воркер
  реально вызывает (#8, @zaartix).
- **Installer** — `write_launchd_plists()` остаётся create-if-missing, но перестал молчать:
  если существующий агент отличается от генерируемого, рядом пишется `.new` и печатается,
  какой интерпретатор и PATH подставил бы этот прогон. Ловит главный случай — `/bin/bash` 3.2
  в plist, который `darkflow-run.sh` не может даже распарсить, при чистом выводе установщика
  (#9, @zaartix).
- **Installer** — `package.json` без поля `name` регистрировал проект в Web UI как строку
  `undefined`: `node -p` печатает её и выходит с нулём, так что `-n`-гард проходил и фолбэк на
  имя каталога не срабатывал (#10, @zaartix).

### Updated routine — codex-раны и честный ci-watch
- **Updated routine** — промпт для `codex exec` передаётся после `--`. Все 22 файла команд
  начинаются с YAML-фронтматтера `---`, clap принимал его за флаг, и каждый ран на
  `engine: codex` умирал с `exit 2` до модели (#6, @zaartix).
- **Updated routine** — `ci-watch` больше не докладывает «зелено» и **не закрывает задачу про
  красный CI**, когда probe A не отработал: без `gh` на PATH блок GitHub Actions пропускался
  молча, `failed` оставался пустым, и управление доходило до зелёной ветки. Ран, где отработали
  только локальные проверки, теперь так и говорит (#7, @zaartix).

### Deps
- **Installer** — `next` 15.5.22 → 15.5.25: критические RCE (`GHSA-p293-qw3h-jr36`,
  `GHSA-2xp9-vwfh-vxw4`). Лок пересобран локально — PR dependabot'а (#12) выкидывал из него
  блок `overrides:` и ронял `pnpm install --frozen-lockfile`.

## [4.44.1] — 2026-08-30

- **Updated routine** — `analytics-review`: событие оплаты — обязательный последний шаг воронки,
  его *количество* меряется в Step 2 как любой другой шаг; хука `revenue.sh` требует только
  *сумма*. Воронка, кончающаяся на `checkout_started`, на вопрос про продажи не отвечает —
  ненастроенное событие оплаты теперь считается инструментальным пробелом высшего приоритета.
  В 4.44.0 формулировка Step 6 читалась так, будто без хука биллинга продаж не видно вовсе.

## [4.44.0] — 2026-08-30

### Updated routine — analytics-review стал разбором воронки, а не сводкой цифр
- **Динамика в двух окнах** — Step 2 гоняет `funnel --prev` дважды: неделя к неделе и
  вчера к позавчера (`--end $YDAY`, потому что сегодня — неполный день и даёт фальшивый обвал
  на каждом шаге). В таблице теперь `Δ WoW` (тренд) и `Δ DoD` (тревога).
- **Step 3.5 «почему шаг проседает»** — самый большой провал не описывается цифрой, а
  открывается браузером (`ego-browser`) на ширине телефона: console errors, 4xx/5xx, мёртвый CTA.
  Плюс ошибки обсервабилити по роутам этого шага. Найденная поломка — инцидент: `--status approved`
  сразу, без порога в 3 рана.
- **Коммиты и гипотезы** — Step 4 сопоставляет `git log --date=short` по дням с дельтами шагов
  (коммит, не трогавший этот путь, лидом не считается) и читает `docs/state/hypotheses.md`:
  открытым ставкам обновляет счётчик, опровергнутые не воскрешает, противоречие подтверждённой
  ставке идёт заголовком.
- **`revenue7d` через хук** — `.darkflow.d/revenue.sh 7` печатает `<amount> <currency>`; нет
  хука → `null`, как раньше. OpenPanel свойства событий не отдаёт, поэтому выручка приходит из
  биллинга. Без хука воронка обрывается на «дошёл до чекаута» и на «почему мало продаж»
  отвечать нечем.

### Updated routine — check-ux
- Порядок флоу задаёт воронка, а не sitemap: команда читает
  `.darkflow.d/state/metrics/analytics.json` и секцию `## Analytics` последнего лога, и первым
  идёт худший шаг. Нет данных — старый порядок.
...
- **Workflow** — шаги `## Plan` теперь обязаны быть **максимально независимыми**: каждый шаг ландится сам по себе, оставляет проект работающим и может быть выполнен отдельным прогоном. Ранний шаг не должен зависеть от позднего, а два шага, которые имеют смысл только вместе, объединяются в один. Это условие того, чтобы задачу вообще можно было резать по прогонам: шаг, который не стоит сам по себе, не может быть точкой остановки.
- **Workflow** — заголовок `## Plan (one step per run)` переименован в `## Plan (multi-run)`, и правило смягчено: `fix-issues` берёт **столько шагов, сколько реально влезает в один прогон** — непрерывный блок от `K+1`, — а не ровно один. Где остановиться, решает сам прогон: шаги, которые суть одно изменение, делаются вместе; шаг, которому нужен задеплоенный предыдущий или прошедшая миграция, откладывается; растущий до нечитаемости диф — повод остановиться.
- **Workflow** — маркер прогресса в комментарии сменился с `Step K/M` на `Progress: K/N` — сколько шагов из скольких закрыто на сейчас, независимо от того, сколько их взял конкретный прогон. Следующий тик читает максимальный `Progress: K/N` и стартует с `K+1`.
- **Updated routine** — `add-issue` требует независимости шагов при составлении плана и предлагает `## Plan (multi-run)`, когда работа явно не влезает в прогон.
- **Docs** — `docs/tasks.md`: правило независимости в строке `Plan`, строка `Plan (multi-run)` в таблице секций шаблона.

## [4.42.0] — 2026-08-26

- **Workflow** — задачу теперь можно закрывать по шагам, за несколько прогонов: заголовок `## Plan (one step per run)` вместо обычного `## Plan` означает, что `fix-issues` берёт **один** шаг за тик — один коммит или PR на шаг — и закрывает задачу только на последнем. Новых сущностей нет: та же секция `## Plan`, те же чекбоксы, отличие только в заголовке; обычный `## Plan` по-прежнему делается целиком за прогон и остаётся значением по умолчанию.
- **Workflow** — прогресс живёт в комментариях задачи (`Step K/M — …`), потому что тело задачи из CLI не редактируется. Следующий прогон считает комментарии, первая строка которых начинается с `Step K/M`, и делает шаг `K+1`. Пока непройденные шаги есть, статус остаётся `approved` — задача сама возвращается в очередь на следующем тике и выигрывает ранжирование, пока не будет доделана.
- **Updated routine** — `add-issue` предлагает этот заголовок, когда работа явно не влезает в один прогон (миграция, рефакторинг по многим точкам вызова); по умолчанию — обычный `## Plan`.
- **Docs** — `docs/tasks.md`: строка про `Plan (one step per run)` в таблице секций шаблона.
...
- **Workflow** — `## What to do` (абзац прозы) заменён на `## Plan` — 2–5 шагов чеклистом с файлами, которые надо тронуть. План объявлен отправной точкой, а не контрактом: `fix-issues` следует ему там, где он ещё верен, и переписывает там, где код ушёл вперёд, сообщая об этом в закрывающем комментарии. Контракт — только acceptance criteria.
- **Workflow** — каждый пункт acceptance criteria обязан называть инструмент, который его решает: команду, отчёт или URL. «Фикс работает» — не критерий, `pnpm build` проходит — критерий. Отдельной секции «how to verify» намеренно нет: критерий, который не говорит, как его проверить, — ещё не критерий, поэтому проверка живёт на самом критерии.
- **Updated routine** — новый шаблон разложен по всем командам, которые заводят задачи: `ads-review`, `analytics-review`, `architecture-review`, `build-optimization`, `check-design`, `check-ux`, `docs-audit`, `observability-check`, `security-audit`, `seo-check`, `uptime-check`, `web-vitals`, `add-issue`, плюс needs-human-задача про mailbox внутри `darkflow-run.sh`.
- **Updated routine** — `fix-issues` читает план и, если останавливается на полпути, перечисляет в комментарии сделанные и несделанные шаги: тело задачи из CLI не редактируется, так что комментарий — единственный след того, как далеко дошло.

## [4.40.0] — 2026-08-21

- **Ops** — новая цель `make web-screen`: тот же билд и запуск, что и `make web`, но сервер живёт в отдельной `screen`-сессии `dfweb` с циклом перезапуска. Причина — вебапп на :5555 регулярно отваливался на часы (рекорд 20 ч, 19→20 августа), и это было незаметно: Next.js получает `SIGTERM`/`SIGINT`, гасится **корректно** и выходит с кодом `0`, поэтому `make`/`pnpm` не печатают ни строчки — в терминале просто молча возвращается промпт. Отдельная сессия отвязывает процесс от tty (сигналы из вкладки до него не долетают), цикл поднимает его через ~2 сек, и простой перестаёт быть многочасовым. Проверено 21 августа: два убийства подряд (12:40:20 и 12:40:49) пережиты автоматически, воркер в соседних тиках даже не заметил пропажи API. Парная цель `make web-screen-stop` гасит сессию вместе с циклом.
- **Ops** — каждое падение вебаппа теперь протоколируется в `~/.darkflow/web-exit.log` строкой `YYYY-MM-DD HH:MM:SS exit=<код>`: `143` — SIGTERM снаружи, `137` — OOM, `0` — запрос на graceful shutdown. Раньше причина смерти не сохранялась нигде, и разбор начинался с нуля при каждом падении.
- **Docs** — README: обе цели в таблице make-таргетов.

## [4.39.1] — 2026-08-17

- **Fix** — `ci-wait.sh` больше не выдаёт **ложный зелёный**, ничего не проверив. После цикла ожидания идёт осознанный перезапрос списка ранов (чтобы поймать второй воркфлоу, регистрирующийся на секунду позже), но результат присваивался в `ids` без проверки на пустоту, а `runs_for_head` глушит ошибки API через `2>/dev/null`. При сбое GitHub список приходил пустым → `for` по нему не выполнялся ни разу → `failed` оставался `0` → `exit 0`, то есть «CI зелёный» без единой строки `watching run`. Поймано на pageradar 2026-08-17 во время `Actions major_outage`: коммит с одним markdown-файлом был отмечен зелёным, тогда как `Build` на нём был красный, и агент отчитался владельцу о зелёном CI. Теперь пустой перезапрос не перетирает уже найденный список, а если проверять действительно нечего — `exit 2` («verified nothing»), а не `0`. Проверено на живом репозитории: скрипт корректно вернул `RED`/`1` там, где прежняя версия промолчала бы нулём.
...
- **Web UI** — карточки статусов на странице проекта выбираются по одной. Мультивыбор накапливал ключи в `?filter=a,b,c`, и все четыре карточки светились одновременно — по картинке не понять, что за список показан ниже. Клик по активной карточке снимает фильтр.
- **Web UI** — «worker offline» в шапке при живом воркере. Воркер шлёт heartbeat раз в тик, а тик — 120 с; окно живости стояло 90 с, так что здоровый воркер числился мёртвым четверть каждого тика. Окно — 300 с (два пропущенных тика).
...
- **Updated routine** — `seo-check`: новый блок «A drop in impressions is not a drop in traffic». Причина: mystize 25 дней разбирал обвал показов (166 → 0 за три дня) и выдал диагноз «закончился discovery-период молодого домена» → эскалация в ссылочное (#34) с критерием приёмки «показы вернулись к > 100/день». Все показы шли с позиций 52–68 при 4–6 кликах за 27 дней, то есть упала метрика ценой в пять кликов.
  - Клики — единственная метрика Performance-отчёта, не задетая логгинг-ошибкой Google 13.05.2025 → 27.04.2026 (impressions/CTR/position врали ~50 недель, история не пересчитана). Показ ниже первой страницы требует, чтобы пользователь долистал: сотни показов в день на позиции 60 — это машина, читающая выдачу, а не аудитория. Запрет: показы с позиции > 20 в заголовке снимка, в теле задачи и в критерии приёмки.
  - Обвал разбирается по официальному дереву Google (`debugging-search-traffic-drops`): algorithmic update · technical · security · spam · seasonality · site moves — с пометкой checked/excluded/not checkable по каждому. Первичные источники (Search Status Dashboard, Data anomalies) — один фетч, раньше не проверялись ни разу.
  - У Search Console API ровно четыре ресурса (`searchAnalytics`, `sitemaps`, `sites`, `urlInspection`) — manual actions и security issues в них нет, поэтому прогон «только по API» не имеет права писать «технические причины исключены, осталось ссылочное».
  - 🚫 «Сайт вышел из песочницы / закончился discovery-период» запрещён как диагноз: ни в одном списке Google его нет, песочницу Google отрицает, а объясняет он что угодно. Вместо него — разбивка падения по запросам.
...
- **Updated routine** — `seo-check`: перенос корпуса уроков SEO (31 урок, `logos/memory/learnings/seo/`) в рутину, которая заводит задачи. Правила жили в `checklists/seo.yml`, но `checklist-review` только отчитывается — аудит их не видел и находил те же дефекты заново либо не находил вовсе. Добавлено девять блоков, каждый на дефекте, реально выкаченном в портфель:
  - **Краулимость — И-цепь из 5 звеньев**, а не список: бот встаёт на первом разрыве, работа ниже разрыва бессмысленна. Ловушки: `Disallow` + `noindex` уничтожают друг друга (чтобы спрятать страницу, её надо сначала открыть), `"use client"`-страница не может экспортировать `metadata` и молча наследует title главной.
  - **`noindex` на целевой странице** — готовый греп с исключением login/account/checkout. sqlformatter.dev месяц стоял вне топ-100 из-за `robots: { index: false }`, поставленного вручную «против каннибализации». Плюс лестница снятия проигравшей страницы: title → перелинковка → canonical → 308 → и только потом noindex.
  - **Canonical**: Next.js не эмитит canonical сам (безусловны только `charset` и `viewport`); canonical со страницы 2+ на страницу 1 — задокументированный антипаттерн; сила сигналов редирект > canonical > ссылки > sitemap; за дубли штрафа нет.
  - **hreflang** — двусторонний или игнорируется целиком; `<loc>` и `href` совпадают посимвольно; в Search Console результат не виден в принципе.
  - **Редиректы**: абсолютный `Location` = ответил прокси (кода в репозитории в проде нет), относительный = ответил фреймворк; `permanent: true` = 308, `redirect()` = 307; `notFound()` при стриминге отдаёт **200** и делает soft-404; отсутствующая DNS-запись на `www` — не 404, а обрыв.
  - **`lastmod`** — единственный дефект, общий для всех семи сайтов, и его первопричина — официальный пример Next.js `lastModified: new Date()`. Диагностическая команда, признаки вранья (одна дата на весь sitemap, кластер в пределах миллисекунд = `mtime` под Docker), правило «правда или молчание», запрет на задачи про `priority`/`changefreq`.
  - **Спрос до генерации страниц** — шаблон умножает страницы, но не спрос; медиана < 50/мес → одна страница вместо N; считать клики с ОДНОЙ страницы, не сумму по шаблону. 35 из 42 страниц sqlformatter были технически безупречны и под нулевым спросом.
  - **Внутренние ссылки** — «every page you care about should have a link from at least one other page», sitemap ссылкой не является; пагинация «1, N−1, N+1, last» и «Load more», удаляющий рёбра; и явный запрет на мифы (3 клика, лимит 100 ссылок, nofollow-скульптинг).
  - **Два статуса GSC** — «Discovered» (обход, НЕ качество) против «Crawled – currently not indexed» (качество); `position` в строке запроса — topmost, а не среднее.
  - **Таблица «что НЕ заводить»** (llms.txt, deprecated `indexed`, каннибализация, `priority`/`changefreq`, «добавить рейтинги», «дописать текста», нит robots.txt) и **критерии приёмки, проверяемые инструментом** — «разметка есть» не критерий, критерий — Rich Results Test определяет тип.
  - Плюс: искать выгрузки GSC в `docs/insights/` до того, как просить данные у человека.
...
- **Updated routine** — `analytics-review` переписана. Гейт шума поднят в шаг 1 и получил числа (`< 100 visitors` или `< 10 входов в воронку` за 7 дней → снапшот и выход): раньше он стоял шагом 4, уже после воронки и трёх сегментных разрезов, то есть срабатывал, когда работа сделана. Сегменты больше не обязательны все три — режется **шаг с наибольшим провалом**, и разрез прекращается, как только сегменты уходят в однозначные числа; прежняя редакция требовала три таблицы и тут же объясняла, что при малом n они ничего не значат. Снапшот пишется **до** дейли-лога, чтобы упавший на полпути прогон не оставлял веб-UI прошлонедельные цифры. `usersTotal` и `revenue7d` перестали быть выдумкой: они не входят в 7-дневное окно воронки, поэтому заполняются только если metrics.md называет событие-источник, иначе `null`. Порог 3 прогонов теперь **исполняется** (`df runs analytics-review`, `rg -l '^## Analytics' docs/logs/`), а не просто упоминается ссылкой. Дедуп расширен со своего источника на все открытые задачи — просадку на `/checkout` мог уже завести `check-ux` или `web-vitals`. Ветка «нет OpenPanel MCP» доопределена: выход без задач вместо «review commits only» с неясным результатом. Убраны квирки OpenPanel API (коды 400/500, `start` vs `start_date`, `interval: month`) и проектная специфика (двоящиеся события, тестовые аккаунты) — первое живёт в схеме MCP-тулов, второе в `docs/state/product/metrics.md`.
- **Worker** — `darkflow-run.sh` экспортирует `DARKFLOW_HEADLESS=1`. Рутины определяли режим, вызывая `AskUserQuestion` и ловя падение; теперь `analytics-review` читает переменную. **Требуется перезапуск воркера** — изменился сам скрипт.
- **Updated routine** — `ads-review` приведена к тому же правилу приоритетов, что и остальные: вместо «`low` не заводить никогда» — floor `minPriority` проекта и запрет надувать приоритет ради него. Две рутины с одной шкалой давали разный ответ на один и тот же вопрос.

## [4.33.1] — 2026-08-13

- **Updated routine** — `checklist-review` больше не рутина, а ручная команда: запись убрана из каталога (`webapp/src/lib/routines.ts`), расписания и диспетчеризации у неё нет. Прогон по чек-листам нужен в конкретный момент — перед запуском, когда возвращаешься к затихшему проекту, после пачки фиксов, — а не раз в неделю по будильнику. Слэш-команда и сами чек-листы ставятся как раньше; в сгенерированном `.darkflow.d/claude.md` и в README она переехала из списка рутин в раздел ручных команд. Строк `RoutineConfig` с этим именем нет ни у одного проекта, чистить нечего.

## [4.33.0] — 2026-08-13

- **New routine** — `checklist-review`: прогон продукта по чек-листам готовности, с аргументом группы (`/darkflow:checklist-review [code|architecture|ux|seo|ads|security|analytics|ops|all]`). Только отчёт: ничего не заводит, ничего не коммитит, пишет `docs/state/readiness.md` перезаписью — это снимок «как сейчас», а не история. Пункт без доказательства получает `n/a`, а не `pass`, чтобы непроверенное не завышало скор. Ставится **выключенной** во всех проектах (`module: "checklist"` — модуль, которого нет ни у одного проекта); включается точечно в Settings → Routines. С `module: null` она сама поднялась бы на всех 19 зарегистрированных проектах на следующем тике.
- **Checklists** — новый глобальный артефакт `~/.darkflow/checklists/*.yml`: 8 групп, 78 пунктов + 5 записей `never_flag` (`templates/darkflow/checklists/`). В проекты не копируется — один версионируемый источник на весь портфель. Каждый пункт несёт `source`: урок из корпуса logos (`seo/NNNN`, `ads/NNNN`) или проект, где дефект реально выехал.
- **Checklists** — у каждой группы два списка, а не один. `never_flag` — известные ложные срабатывания, которые аудиты поднимают снова и снова, а чинить их не надо: `/llms.txt` (урок 0015 прямо это говорил 29.07, но 10.08 рутина завела задачи, 12.08 файл выкатили), баг robots.txt, который аудиты метят «high» в 4 из 7 сайтов, каннибализация (правильный ответ почти всегда «не трогать») и `contents[].indexed` из Sitemaps API — поле помечено deprecated и равно нулю у всех, а рутины три месяца заводили по нему задачи. Это ровно тот механизм, которого не хватало: `seo/NOTES.md` формулирует правило «если урок меняет поведение агента, урока мало — правило надо коммитить в рутины».
- **Installer** — в `ALL_DF_COMMANDS` добавлен `checklist-review`; появился `ALL_DF_CHECKLISTS` и выкачивание групп в `~/.darkflow/checklists/` внутри `install_global_helpers()` (у raw GitHub нет globbing, поэтому список групп явный). Перезапуск воркера не нужен: `darkflow-run.sh` не менялся, диспетчеризация идёт по имени рутины.
- **Docs** — в `CLAUDE.md` поправлено дерево репозитория: `update.sh` и `setup-labels.sh` там описаны, но в репозитории их давно нет — остался только `install.sh`.
...
- **Fix** — `run_checklist --fix` вызывался в конце установки независимо от `--dry-run` и чинил «недостающее» по-настоящему: создавал `docs/logs/`, `docs/state/` и копировал шаблоны. То есть предпросмотр делал ровно то, ради чего его и запускают, чтобы не делать. Теперь в режиме `--dry-run` он печатает, сколько пунктов починил бы, и выходит.
- **Fix** — в сверке `docs/` (I1) `rmdir` пустого каталога и удаление `.gitkeep` выполнялись и при `--dry-run`. Пустой `docs/product/` в реальном проекте оказался удалён во время предпросмотра. Оба действия теперь под флагом.
- Проверено: `git status --porcelain --untracked-files=all` до и после `--dry-run` на реальном проекте совпадает побайтово.

## [4.29.0] — 2026-08-11

Шаг 9 слияния с Deckbook (H): суточный housekeeping в воркере.

- **New routine** — `housekeeping`, ежедневно в 5:00. Целиком bash внутри воркера: ни агента, ни файла команды, ни токенов — как `ci-watch`. Три починки: (1) задача в `in-progress` без активности дольше 4ч получает комментарий и возвращается в `approved`; (2) `HEAD`, оставленный на фиче-ветке, возвращается на базовую; (3) `git worktree prune` и удаление слитых веток. Ничего не коммитит, не пушит и не выбрасывает.
- **Worker** — восстановление `HEAD` — не косметика. По A7 каждая рутина пишет в репозиторий, и пока `HEAD` не на базовой ветке, **все** эти записи ложатся не туда. Вернуть задачу, не вернув чекаут, чинит половину поломки.
- **Worker** — проверка «грязный ли чекаут» игнорирует `docs/logs/` и `docs/state/`: под стратегией `pr` аудиты ничего не коммитят, их файлы и должны там лежать. Без исключения весь проход был бы мёртв с рождения — проект без открытых задач неделями не открывает PR, лог висит незакоммиченным, чекаут вечно выглядит грязным, и ничего не чинится ровно тогда, когда починка нужна.
- **Fix** — найдено при проверке: `git status --porcelain` схлопывает неотслеживаемый каталог до `docs/`, и исключение не срабатывало — свежий дневной лог читался как посторонняя работа и блокировал все починки. Добавлен `--untracked-files=all`.
- **Docs** — `docs/housekeeping.md`. Оговорка к плану: «Dark Flow не имеет восстановления зависших задач вообще» — неверно, `revive_stuck_issues` (1ч) существует и вызывается в начале каждого прогона `fix-issues`. Но срабатывает он только когда `fix-issues` запущен и у него есть работа. Housekeeping — сетка под ним: идёт независимо и оставляет комментарий.

## [4.28.0] — 2026-08-11

Шаг 8 слияния с Deckbook: установщик приводит существующий проект к новому layout.

- **Installer** — I1: `install.sh` сверяет `docs/` с эталоном при каждом запуске. Старые `docs/spec/`, `docs/product/`, `docs/design/` переезжают под `docs/state/` (вместе с `architecture.md` → `state/arch.md` и `hypotheses.md` → `state/hypotheses.md`), содержимое сливается с уже существующим, коллизия имён получает суффикс `.old` — ничего не перезаписывается. Частично мигрированный проект догоняется без вложенности вроде `state/spec/spec`.
- **Installer** — A8: один архив, `docs/_archive/`. Туда уезжают папки снимков `docs/insights/<область>/`, которые после A3 никто больше не обновляет. `insights/qualitative/` остаётся на месте: интервью и записи сессий — исходный материал, а не суточный вывод рутины. Спрашивает подтверждение в интерактивном режиме. **Ничего не удаляется** — только перемещается, так что `git status` показывает каждый файл до коммита.
- **Installer** — A11: удаляются осиротевшие `.darkflow.d/state/metrics/*.json` — всё, кроме трёх файлов, которые воркер действительно пересылает (`analytics`, `security`, `architecture`). Это единственное удаление во всей сверке, и касается оно только производных данных, лежащих вне git. Файл, который никто больше не пишет, иначе оставил бы виджет замороженным на старом значении.
- **Docs** — правило архива записано в `docs/README.md`: под `state/` не архивируется ничего (текущее значение перезаписывается на месте, старое лежит в git), дневные логи туда тоже не уезжают.
- **Отступление от плана**: I1 предписывал «увезти устаревший layout в `_archive/`». Для `docs/product/` и `docs/spec/` это скрыло бы живые документы, и следующий аудит переписал бы их с нуля. У этого layout есть прямой преемник, поэтому он **мигрируется**, а архивируется то, у чего преемника нет — снимки `insights/`.
...
- **Workflow** — A3: снимки `docs/insights/<область>/YYYY-MM-DD.md` заменены одним документом в день — `docs/logs/YYYY-MM-DD.md`. Каждая рутина дописывает свою секцию: `## Security`, `## Analytics`, `## Performance`, `## Changes` и так далее (12 секций, `observability-check` и `web-vitals` делят одну `## Performance` — разные источники, разная частота, один раздел). Правило жёсткое: **чистый прогон не дописывает ничего** — ни секции, ни строчки «всё в порядке». На это опирается порог. Логи не ротируются: увезти старые значило бы стереть ту самую историю, по которой порог считается.
- **Workflow** — P3: инцидент и предложение больше не одно и то же. Сломано прямо сейчас → задача с первого раза, `--status approved`. Улучшение → `proposed` и очередь триажа в веб-интерфейсе. **Не `needsHuman`** — в Dark Flow этот флаг значит «агент застрял» и взаимоисключим с `approved`.
- **Workflow** — P4: наблюдение становится задачей после **3 прогонов подряд** или **2 независимых источников**. Считается по *прогонам*, а не по файлам: чистый прогон не пишет секцию, поэтому по одним логам не отличить «запускалась 3 раза и все 3 видела» от «запускалась 10 раз, видела 3». Добавлены `GET /api/projects/routine-logs` и `df runs <routine> --limit N` — вторая половина этого счёта. Прогон без секции **обрывает серию**.
- **Workflow** — A7: что рутина коммитит. Два предусловия в любом режиме: `HEAD` на базовой ветке (иначе не писать ничего и остановиться) и стадирование только своих путей, поимённо — `git add -A` здесь неверен всегда. Дальше по стратегии: **direct** — рутина сама коммитит и пушит; **pr** — не коммитит и не пушит вовсе, файл ждёт ближайшего PR, а `fix-issues` дополнительно стадирует `docs/logs/` и `docs/state/`, если они изменились.
- **Installer** — `docs/logs/` создаётся при установке и проверяется чеклистом. Каталоги снимков по модулям (`insights/analytics`, `insights/search-console`, `insights/seo-audit`, `insights/ads`) больше не создаются. `insights/qualitative/` остаётся: интервью и записи сессий — исходный материал, а не суточное наблюдение.

## [4.25.1] — 2026-08-11

Шаг 5 слияния с Deckbook (A10) — оказался почти целиком уже сделанным.

- **Worker** — A10 исходил из того, что глобальный семафор пускает две рутины одного проекта разом. Это не так: пер-проектный локер `.darkflow.d/state/.lock` уже есть, берётся **до** глобального слота всеми входными точками (подоболочка watch-цикла, `--dry-run`, ручной запуск), захватывается атомарно через `mkdir`, переоткрывается при мёртвом владельце, а рутины внутри него идут последовательно. Занятый проект уже пропускал тик. Новый локер не нужен — предпосылка A10 опровергнута, требование выполнено.
- **Worker** — единственная настоящая дыра: ручной `darkflow-run.sh <routine>` при занятом проекте выходил **молча** с кодом 0, и человек не мог отличить «уже работает» от «ничего не сделал». Теперь печатает, какой PID держит проект.
- **Docs** — гарантия «одна сессия на проект» и причина выбора `mkdir` вместо `set -o noclobber` записаны прямо над локером, чтобы следующий читатель не выводил это заново.

## [4.25.0] — 2026-08-11

Шаг 4 слияния с Deckbook: команды слиты, плюс P5 и P7.

- **Updated routine** — C1: `security-audit` вобрал `vulnerability-check`. Один еженедельный проход на Opus: сначала алерты GitHub (Dependabot, code scanning, secret scanning), потом чтение кода. Цена — время реакции: advisory от Dependabot теперь может пролежать до семи дней вместо одного. Метрики: `security.json` вобрал `vulnerabilities.json`.
- **Updated routine** — C2: `architecture-review` вобрал `code-health`. Шаг fallow внутри — опциональный, потому что покрывает только TS/JS, а остальной разбор языконезависим. На Python-репозитории шаг просто пропускается, и это нормальный исход, а не деградация. Метрики: `architecture.json` вобрал `code-health.json`.
- **New routine** — C4: три дизайн-команды стали двумя. `check-design` — визуальное качество, производительность интерфейса, готовность к продакшену. `check-ux` — ключевые флоу, пройденные в настоящем браузере на 390×844 и 1440×900 (это и есть P8). Разделение по смыслу: первая смотрит на поверхности, вторая пользуется продуктом. `check-ux` не подменяет проход чтением кода: если URL не задан — не пишет ничего.
- **Workflow** — P7: аудиты дедуплицируют по **всем** задачам проекта, а не только по своему `--source`. Одна и та же проблема приходит в два аудита с разных сторон, и фильтр по источнику заводил её дважды. Добавлен флаг `df task list --format compact` — строка на задачу, достаточно дёшево, чтобы прочитать весь список перед заведением.
- **Web UI** — P5: у проекта появилось одно новое поле — Staging URL (миграция `20260811120000_add_project_staging_url`). Паспорта продукта и колонки «ключевые страницы» не будет: командам, которым нужны страницы для обхода, их даёт `sitemap.xml`.
- **Docs** — обнаружено при слиянии: из 15 файлов `.darkflow.d/state/metrics/*.json`, которые пишут команды, воркер пересылает в webapp ровно три — `analytics.json`, `security.json`, `architecture.json`. Остальные двенадцать пишутся в никуда. Поэтому `check-design` и `check-ux` метрик не пишут вовсе.
