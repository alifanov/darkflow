...
Weekly audit of the project's build and deploy pipeline — maps how the project actually builds and ships, finds bottlenecks and inefficiencies, and creates proposed tasks with concrete optimization proposals.
...
Tasks with: `status=proposed`, `source=build`, `priority=high` / `medium` (`low`-impact optimizations are noted in the snapshot, not filed)
...
- Only creates tasks for significant, independent opportunities — trivially related fixes (e.g., `.dockerignore` + layer ordering) are grouped into a single task.
- Acceptance criteria in each task should be **measurable**: "CI install step drops from ~90s to ~15s on cache hit", not "build should be faster".
- Opportunities already tracked in open tasks are skipped to avoid duplicates.
