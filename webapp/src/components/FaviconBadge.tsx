...
const BASE_SVG_WITH_COUNT = (label: string) =>
  `<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64">
  <rect width="64" height="64" rx="14" fill="#ef4444"/>
  <text x="32" y="46" text-anchor="middle" font-family="Arial,sans-serif" font-size="${label.length > 1 ? "36" : "44"}" font-weight="bold" fill="white">${label}</text>
</svg>`;
...
async function updateFavicon() {
  try {
    ...
    const svg =
      count > 0
        ? BASE_SVG_WITH_COUNT(count > 99 ? "99+" : String(count))
        : BASE_SVG;
